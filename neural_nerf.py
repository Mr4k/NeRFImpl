...
class NerfModel(torch.nn.Module):
    def __init__(self, scale, device):
        ...
        self.scalers = {
            pos_input_dims: torch.pow(2, torch.arange(0, pos_input_dims, 1, device=device)).repeat_interleave(3),
            dir_input_dims: torch.pow(2, torch.arange(0, dir_input_dims, 1, device=device)).repeat_interleave(3)
        }
    ...
    def forward(self, pos_input, dir_input):
        pos_embedding = self.embed_tensor(pos_input / self.scale, self.l_pos, self.device)
        ...
        dir_input = embed_tensor(dir_input, self.l_dir, self.device)
